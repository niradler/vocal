...
class TTSRequest(BaseModel):
    ...
    response_format: str = Field(
        "wav", description="Audio format (currently only 'wav' supported)"
    )
...
@router.post(
    ...
    responses={
        200: {"content": {"audio/wav": {}}, "description": "Audio file in WAV format"}
    },
)
...
async def list_voices():
    ...
    try:
        ...
        voice_infos = [
            VoiceInfo(id=v.id, name=v.name, language=v.language, gender=v.gender)
            for v in voices
        ]
