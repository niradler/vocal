...
class FasterWhisperAdapter(STTAdapter):
    ...
    async def load_model(
        ...
        **kwargs,
        ...
        self.compute_type = (
            settings["compute_type"] if compute_type == "auto" else compute_type
        )
        ...
        logger.info(
            f"Loading model from {model_path} on {self.device} "
        ...
        if self.device == "cuda":
            try:
                ...
                if torch.cuda.is_available():
                    ...
                    info["vram_total_gb"] = torch.cuda.get_device_properties(
                        0
                    ).total_memory / (1024**3)
            except Exception:
                pass
