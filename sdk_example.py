...
def main():
    ...
    try:
        ...
        print(f"   ERROR: {e}")
        print(
            "   Make sure API is running: uv run uvicorn vocal_api.main:app --port 8000"
        )
        return
    ...
    for model in models_response["models"][:5]:
        ...
        print(
            f"       {model['name']} - {model['parameters']}, {model['recommended_vram']}"
        )
    ...
    try:
        result = client.audio.transcribe(
            file=audio_file, model=model_id, response_format="json"
        )
