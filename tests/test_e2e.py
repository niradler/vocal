...
def api_server():
    ...
    for i in range(max_retries):
        ...
        server_process.kill()
        raise RuntimeError(
            f"Failed to start API server after {max_retries * retry_delay}s"
        )
...
class TestModelManagement:
    ...
    def test_download_model(self, client, test_model):
        ...
        assert model_info["status"] in ["available", "downloading"], (
            "Model should be available or downloading"
        )
...
class TestAudioTranscription:
    ...
    def test_transcribe_short_audio(self, client, test_model, test_assets):
        ...
        assert (
            expected_text.lower() in transcribed.lower()
            or transcribed.lower() in expected_text.lower()
        ), f"Transcription mismatch. Expected: '{expected_text}', Got: '{transcribed}'"
    ...
    def test_transcribe_with_language_specification(
        self, client, test_model, test_assets
    ):
        """Test transcription with specified language"""
        audio_file = test_assets["audio_dir"] / "Recording.m4a"

        result = client.audio.transcribe(
            file=audio_file, model=test_model, language="en"
        )
    ...
    def test_transcribe_json_format_with_segments(
        self, client, test_model, test_assets
    ):
        ...
        result = client.audio.transcribe(
            file=audio_file, model=test_model, response_format="json"
        )
    ...
    def test_synthesize_with_speed(self, client):
        ...
        try:
            ...
            if len(fast_audio) < len(normal_audio):
                print(
                    f"  Speed reduction verified ({len(normal_audio) / len(fast_audio):.2f}x)"
                )
            else:
